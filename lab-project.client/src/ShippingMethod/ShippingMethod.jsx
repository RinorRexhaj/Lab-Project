...
const ShippingMethod = () => {
  ...
  const fetchShippingMethods = async () => {
    try {
      const response = await axios.get('https://localhost:7262/shippingmethods');
  ...
  const addShippingMethod = async (method) => {
    try {
      const response = await axios.post('https://localhost:7262/shippingmethods', { name: method });
  ...
  const editShippingMethod = async (oldMethod, newMethod) => {
    try {
      await axios.put(`https://localhost:7262/shippingmethods/`, { name: oldMethod, newName: newMethod });
      setShippingMethod(shippingMethod.map(m => {
  ...
  const deleteShippingMethod = async (method) => {
    try {
      await axios.delete(`https://localhost:7262/shippingmethods/${method}`, { name: method });
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-xl">
      <h1 className="text-3xl mb-4 font-medium text-center text-white bg-gradient-to-r from-blue-400 to-indigo-500 py-2 rounded-lg shadow-md">Manage Shipping Methods</h1>
          ...
          <tbody>
            {shippingMethod.map((method, index) => (
              <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-200'}`}>
                <td className="border-b border-gray-200 p-3">{editingIndex === index ? (
                  <input
                    type="text"
                    value={editMethod}
