...
const Product = ({id, name, category, price, openModal, setModalAction, setEditData, setDeleteId }) => {
    return (
        <div className="relative w-full py-6 px-8  sm:px-4 flex md:flex-wrap items-center justify-between gap-4">
          <div className="w-1/3 md:w-3/5 text-black font-medium text-sm flex items-center gap-5 sm:gap-2">
            <ProductImage src={"https://localhost:7262/Products/image/"+id} alt={name}/>
          ...
          <div className="w-1/4 flex items-center gap-4  md:gap-2">
            <button className="h-10 p-2 flex items-center gap-2 rounded-md font-medium sm:text-sm bg-blue-500 hover:bg-blue-600 duration-150 ease-linear text-white" onClick={() => { 
              openModal();
              setEditData({
                id: id,
                name: name,
                category: category,
                price: price,
                image: "https://localhost:7262/Product/image/"+id
              });
              setModalAction("EDIT");
          }}><FontAwesomeIcon icon={faPenToSquare}/> EDIT</button>
            <button className="h-10 p-2 flex items-center gap-2 rounded-md font-medium  sm:text-sm bg-red-500 hover:bg-red-600 duration-150 ease-linear text-white" onClick={() => {
              setDeleteId(id);
              openModal();
