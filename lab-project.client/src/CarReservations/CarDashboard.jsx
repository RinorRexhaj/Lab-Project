...
import {faBriefcase, faCar,  faMoneyBillTrendUp, faUser} from "@fortawesome/free-solid-svg-icons";
...
const CarDashboard = ({token, setToken, user,cars, setCars, carsFilter}) => {
    ...
    useEffect(() => {
      ...
      const generateSalesData = (days) => {
        ...
        const filteredReservations = reservations.filter(reservation => new Date(reservation.startDate) >= pastDate);
        ...
        for (let i = 0; i < days; i++) {
          ...
          const daySales = filteredReservations
            .filter(reservation => new Date(reservation.startDate).toLocaleDateString() === dateString)
            .reduce((total, reservation) => total + (cars.find(car => car.id === reservation.carID)?.price * reservation.days), 0);
          ...
          <div>
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-6 bg-white rounded-lg p-2">
                  <button onClick={() => setTimePeriod('7')} className={`font-bold p-1 rounded-lg ${timePeriod === '7' ? 'bg-blue-500 text-white' : 'hover:bg-blue-500 hover:text-white transition-all'}`}>Last 7 days</button>
                  <button onClick={() => setTimePeriod('30')} className={`font-bold p-1 rounded-lg ${timePeriod === '30' ? 'bg-blue-500 text-white' : 'hover:bg-blue-500 hover:text-white transition-all'}`}>Last 30 days</button>
                  <button onClick={() => setTimePeriod('90')} className={`font-bold p-1 rounded-lg ${timePeriod === '90' ? 'bg-blue-500 text-white' : 'hover:bg-blue-500 hover:text-white transition-all'}`}>Last 90 days</button>
                </div>
            </div>
            <div className="flex mt-15 gap-5 justify-between">
                <div className="bg-white shadow rounded-lg flex items-center p-5 gap-5">
                    <FontAwesomeIcon icon={faBriefcase} className="text-[35px] text-blue-500 bg-blue-200 p-5 rounded-full"/>
                    <div className="flex flex-col pr-12">
                        <p className="text-sm text-neutral-400 font-bold">BOOKINGS</p>
                        <h1 className="text-3xl font-medium text-black">{filteredBookings}</h1>
                    </div>
                </div>
                <div className="bg-white shadow rounded-lg flex items-center p-5 gap-5">
                    <FontAwesomeIcon icon={faMoneyBillTrendUp} className="text-[35px] text-green-500 bg-green-200 p-5 rounded-full"/>
                    <div className="flex flex-col pr-2">
                        <p className="text-sm text-neutral-400 font-bold">SALES</p>
                        <h1 className="text-3xl font-medium text-black">{filteredSales.toFixed(2)} €</h1>
                    </div>
                </div>
                <div className="bg-white shadow rounded-lg flex items-center p-5 gap-5">
                    <FontAwesomeIcon icon={faCar} className="text-[35px] text-orange-500 bg-orange-200 p-5 rounded-full"/>
                    <div className="flex flex-col pr-12">
                ...
                <div className="bg-white shadow rounded-lg flex items-center p-5 gap-5">
                    <FontAwesomeIcon icon={faUser} className="text-[35px] text-amber-500 bg-amber-200 p-5 rounded-full"/>
                    <div className="flex flex-col pr-12">
            ...
            <div className="flex gap-5 mt-10 justify-between">
                <div className="bg-white rounded-lg shadow w-[585px] p-5">
                    <h1 className="font-bold text-3xl text-neutral-600">Today's Reservations</h1> 
                    <div className="">
                        {todaysReservations.map((reservation) => {
                            ...
                            return (
                                user && (
                                    <div key={reservation.rentalID} className="border-b border-gray-200 py-2">
                    ...
                    <span>{count !== 1 ? `There are ${count} reservations awaiting for today` : `There is ${count} reservation awaiting for today`}  </span>
                </div>
                <div className="bg-white rounded-lg shadow w-[560px] p-5">
                    <h1 className="font-bold text-3xl text-neutral-600">Reservation Duration Summary</h1>
            ...
            <div className="mt-10 bg-white rounded-lg shadow p-5">
              <h1 className="font-bold text-3xl text-neutral-600">Sales Over The Last {timePeriod} Days</h1>
